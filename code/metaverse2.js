...
function loadGLTFModel(url, onProgress) {
...
class App {
    ...
    async _setupModel() {
        ...
        try {
            // map 모델 로딩
            const mapGltf = await loadGLTFModel("./data/sc.glb", (xhr) => {
            ...
            await Promise.all([
                this.loadMaruModel(),
                this.loadXbotModel("./data/Xbot.glb", 'friend_crash', -91, 0, -775),
                this.loadXbotModel("./data/Xbot.glb", 'friend_hurt', -209, 1, -1350, Math.PI / 2, Math.PI / 2),
                this.loadXbotModel("./data/Xbot.glb", 'teacher', 2300, 30, 60),
                this.loadXbotModel("./data/Xbot.glb", 'rector', 225, 1, -1682),
                this.loadXbotModel("./data/Xbot.glb", 'game_friend', -705, 1, -690, Math.PI / 2),
                this.loadXbotModel("./data/Xbot.glb", 'teacher', -104, 0, -160, Math.PI / 4),
                this.loadXbotModel("./data/Xbot.glb", 'casher', 0, 0, -2300),
                this.loadXbotModel("./data/Xbot.glb", 'default', -0.5, 10, -9)
            ]);

            loadingElem.style.display = 'none'; // 모든 모델 로딩 완료 후 로딩 인디케이터 숨김
    ...
    async loadMaruModel() {
        const maruGltf = await loadGLTFModel("./data/maru.glb");
        const support = maruGltf.scene;
        ...
        const animationsMap = {};
        maruGltf.animations.forEach((clip) => {
    ...
    async loadXbotModel(url, type, x, y, z, rotX = 0, rotZ = 0) {
        const xbotGltf = await loadGLTFModel(url);
        const npc = xbotGltf.scene;
        this._scene.add(npc);
        ...
        this._mixers.push(mixer);
        const animationsMap = {};
        xbotGltf.animations.forEach((clip) => {
            animationsMap[clip.name] = mixer.clipAction(clip);
        });
